...
if ($what == 'thedate') {
    ...
    if ($request->GetVar('frm_paycat1', 'post') !== $request->undefined) {
        $paycat = $request->GetVar('frm_paycat1', 'post');
    } 
    $smarty->assign('tpl_income', $account->getlist($thestart, $theend, $paycat));
	...
	if ($request->GetVar('frm_paycat2', 'post') !== $request->undefined) {
        $paycat = $request->GetVar('frm_paycat2', 'post');
    } 
    $smarty->assign('tpl_income', $account->getlist($thestart1, $theend1, $paycat));	
} else {
    list($month, $year) = split('[/]', $theend);
    $theend = "$year-$month-01 00:00:00";
    list($month, $year) = split('[/]', $thestart);
    $thestart = "$year-$month-01 00:00:00";
    if ($request->GetVar('frm_paycat3', 'post') !== $request->undefined) {
        $paycat = $request->GetVar('frm_paycat3', 'post');
    } 	
    $smarty->assign('tpl_income', $account->getlist($thestart, $theend, $paycat));
} 

$smarty->assign('tpl_thepaycat', $paycat);

$smarty->display('list_income.tpl');
