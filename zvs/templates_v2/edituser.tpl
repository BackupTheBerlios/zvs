...
<!--
  ...
  function submitform(){
	...
	function deluser(id, name) {
		var check;
			check = confirm("##REALLY_DELETE## ##USER##: \""+ name +"\"");
		if (check) {
			document.user.frm_userid.value = id;
...
<%strip%>
<div class="box750">
	<h2><span>##ADMINISTER_USER##</span></h2>
	<div class="table">
		   ...
		   <%/if%>
			<%section name=user loop=$tpl_user%>
				<tr <%if $tpl_editid eq $tpl_user[user].userid%>class="ListHighlight"<%else%>class="ListL<%$tpl_user[user].color%>" onMouseOver="this.className='ListHighlight'" onMouseOut="this.className='ListL<%$tpl_user[user].color%>'"<%/if%>>
					...
					<td>
						<%if $tpl_editid eq $tpl_user[user].userid%>
					...
					<td>
						...
						<%else%>
							<a href="javascript:edituser(<%$tpl_user[user].userid%>);" class="dotted"><%$tpl_user[user].login%></a>
						<%/if%>
					</td>			
					<td>
						<%if $tpl_editid eq $tpl_user[user].userid%>
