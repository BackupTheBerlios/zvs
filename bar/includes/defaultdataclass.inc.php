...
* @version $Id: defaultdataclass.inc.php,v 1.1 2004/11/03 15:31:33 ehret Exp $
*/
class DefaultData {
    /**
    * DefaultData::load()
    ...
    */
    function setfield($id, $value)
    {
        ...
        $query = sprintf("SELECT fieldtype, default_name " . "FROM $tbl_default " . "WHERE pk_default_id = %s ",
            $id
            );
        ...
        if (!$result) {
            ...
            $query = sprintf("SELECT updated_date " . "FROM $tbl_hotel_default " . "WHERE pk_fk_hotel_id = %s " . "AND pk_fk_default_id = %s ",
            ...
            if ($rows == 0) {
                $query = sprintf("INSERT INTO $tbl_hotel_default " . "(pk_fk_hotel_id, pk_fk_default_id, $field, " . "fk_inserted_user_id, inserted_date) " . "VALUES (%s, %s, %s, %s, NOW()) ",
                    $request -> GetVar('hotelid', 'session'),
                    $id,
                    $inputvalue,
                ...
                $query = sprintf("UPDATE $tbl_hotel_default " . "SET $field = %s, " . "fk_updated_user_id = %s, " . "updated_date = NOW(), " . "WHERE pk_fk_hotel_id = %s " . "AND pk_fk_default_id = %s ",
                    $inputvalue,
                    $request -> GetVar('uid', 'session'),
                    $request -> GetVar('hotelid', 'session'),
                    $id
