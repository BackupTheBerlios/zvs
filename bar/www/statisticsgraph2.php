...
if ($what == 'thedate') {
    ...
    $data = $statistics->getByHour($thestart, $theend, $thecat1, $from_clock1, $till_clock1);
    ...
    $data = $statistics->getByHour($thestart1, $theend1, $thecat2, $from_clock2, $till_clock2);
    ...
    $data = $statistics->getByHour($thestart, $theend, $thecat3, $from_clock3, $till_clock3);
...
for ($i=0; $i<count($data); $i++){
 $gdata[$data[$i]['hour']] = $data[$i]['price'];
}
...
$graph->title->Set("Umsatz pro Stunde $timeline");
$graph->title->SetFont(FF_FONT1, FS_BOLD);
$graph->yaxis->title->SetFont(FF_FONT1, FS_BOLD);
$graph->xaxis->title->SetFont(FF_FONT1, FS_BOLD);
$graph->xaxis->SetLabelAngle(90); 
$graph->xaxis->title->Set("Uhr");
$graph->yaxis->title->Set("Umsatz [EUR]");
